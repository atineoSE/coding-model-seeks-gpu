...
offering, recording its price (as ``usd_per_node_hour``) and provider.
"""
...
def reduce_offerings_to_nodes(offerings: list[dict]) -> list[dict]:
    ...
    Rules (baked in here and ONLY here):
      ...
      * per ``gpu_name`` pick the offering with the minimum ``price_per_hour``,
        recording that offering's ``provider``.

    Returns a list of ``{"gpu_name", "usd_per_node_hour", "provider"}`` dicts,
    sorted by ``SERVING_NODES`` order, omitting nodes with no 8x offering.
    Robust to older-schema dicts that may lack keys other than
    ``gpu_name``/``gpu_count``/``vram_gb``/``price_per_hour``/``provider``.
    """
    ...
    for offering in offerings:
        ...
        if current is None or price < current["price_per_hour"]:
            cheapest[gpu_name] = {
    ...
    return [
        {
            "gpu_name": name,
            "usd_per_node_hour": cheapest[name]["price_per_hour"],
            "provider": cheapest[name]["provider"],
        }
