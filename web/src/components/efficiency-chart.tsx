...
interface ChartPoint extends EfficiencyTrendPoint {
  closedLabel: string;
  openLabel: string;
  isFirst: boolean;
  isLast: boolean;
  ...
  return (
    <div className="rounded-lg border bg-background p-3 shadow-sm">
      ...
      {payload.map((entry) => {
        ...
        const modelName =
          entry.dataKey === "closedSourceCostPerTask"
            ? point.closedSourceModel
            : point.openSourceModel;
        const isOpen = entry.dataKey === "openSourceCostPerTask";
        return (
...
function ClosedLabel(props: Record<string, unknown>) {
...
function OpenLabel(props: Record<string, unknown>) {
  const { x, y, value } = props as { x: number; y: number; value: string };
  const point = (props as { payload?: ChartPoint }).payload;
  if (!value) return null;
  const anchor = point?.isFirst ? "start" : point?.isLast ? "end" : "middle";
...
export function EfficiencyChart({ data, categoryDisplayName }: EfficiencyChartProps) {
  ...
  const allCosts = data.flatMap((p) => [p.closedSourceCostPerTask, p.openSourceCostPerTask]);
  const maxCost = Math.max(...allCosts);
  const yDomain: [number, number] = [0, Math.ceil(maxCost * 1.2 * 100) / 100];
      ...
      <CardContent>
        <ChartContainer config={chartConfig} className="aspect-auto h-[300px] sm:h-[380px] w-full">
          ...
          >
            ...
            <Line
              type="monotone"
              dataKey="closedSourceCostPerTask"
              stroke="var(--chart-1)"
              strokeWidth={2}
              dot={{ r: 4 }}
              name="Closed Source"
